...
1. **Insights System**: Auto-extracts and stores insights with deduplication via `services/insights_extraction_service.py`
2. **On-Demand Analysis**: Smart threshold-based decisions in `services/ondemand_analysis_service.py`
3. **Unified Architecture**: All endpoints use shared behavior analysis through OnDemandAnalysisService
4. **Memory Integration**: 4-layer memory system (working, episodic, semantic, procedural) fully operational
